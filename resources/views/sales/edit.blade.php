...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h1 class="h3 mb-3"><strong>Edit Penjualan</strong></h1>
            ...
            <div class="card-body">
                <form action="{{ route('sales.update', $sales->id) }}" method="POST">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-3">
                            <label>Tanggal</label>
                            <input type="date" name="date" value="{{ $sales->date }}" class="form-control" required>
                        </div>
                        ...
                        <div class="col-md-3 member-field">
                            ...
                            <select name="member_id" class="form-select select2">
                                ...
                                @foreach ($members as $t)
                                    <option value="{{ $t->id }}" {{ $sales->member_id == $t->id ? 'selected' : '' }}>
                                        {{ $t->name }} - {{ $t->code }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3 customer-field">
                            ...
                            <select name="customer_id" class="form-select select2">
                                ...
                                @foreach ($customers as $c)
                                    <option value="{{ $c->id }}"
                                        {{ $sales->customer_id == $c->id ? 'selected' : '' }}>
                                        {{ $c->name }}
                                    </option>
                                @endforeach
                            </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-2">
                                        ...
                                        <input type="text" name="subtotal" id="subtotal"
                                            value="{{ number_format($sales->subtotal) }}" class="form-control" readonly>
                                    </div>

                                    <div class="mb-2">
                                        <label>Diskon</label>
                                        <input type="text" name="discount" id="discount"
                                            value="{{ number_format($sales->discount) }}" class="form-control">
                                    </div>

                                    <div class="mb-2">
                                        <label>Biaya Lain-lain</label>
                                        <input type="text" name="expenses" id="expenses"
                                            value="{{ number_format($sales->expenses) }}" class="form-control">
                                    </div>

                                    <div class="mb-2">
                                        <label>Pajak</label>
                                        <input type="text" name="taxes" id="taxes"
                                            value="{{ number_format($sales->taxes) }}" class="form-control">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-2">
                                        ...
                                        <input type="text" name="grand_total" id="grand_total"
                                            value="{{ number_format($sales->grand_total) }}" class="form-control" readonly>
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select name="account_id" class="form-control select2">
                                            @foreach ($payment_gateways as $p)
                                                <option value="{{ $p->id }}"
                                                    {{ $sales->account_id == $p->id ? 'selected' : '' }}>
                                                    {{ $p->code }} - {{ $p->name }}
                                                </option>
                                            @endforeach
                                        </select>
...
@section('script')
    <script>
        $(function() {
            ...
            let existingItems = @json($sales->product);
